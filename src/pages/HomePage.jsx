...
function ExploreCard({ card, didDrag, navigate }) {
  return (
    <div
      onClick={() => { if (!didDrag.current) { localStorage.setItem('returning_to_home', 'true'); navigate(card.to) } }}
      style={{
        width: '100%',
        height: '200px',
        borderRadius: '22px',
    ...
    >
        ...
        <div
          ...
          {upcoming.map((ev) => (
            <button key={ev.id} type="button"
              onClick={() => { localStorage.setItem('returning_to_home', 'true'); navigate(`/events/${ev.id}`, { state: { event: ev } }) }}
          ...
          {announcementItems.map((item) => {
            ...
            return (
              <button
                key={item.id}
                type="button"
                onClick={() => { localStorage.setItem('returning_to_home', 'true'); navigate(`/news/${item.id}`, { state: { item } }) }}
                style={{
      ...
      <div style={{ paddingTop: '30px', paddingLeft: '24px', paddingRight: '24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginTop: '14px' }}>
          ...
          <button type="button" onClick={() => { localStorage.setItem('returning_to_home', 'true'); navigate('/last-sunday') }} style={qaCard}>
            <Play size={24} color="#ffffff" strokeWidth={1.75} />
            <div><p style={qaLabel}>Last Sunday</p><p style={qaSub}>Sermon</p></div>
          </button>
          <button type="button" onClick={() => { localStorage.setItem('returning_to_home', 'true'); navigate('/midweek') }} style={qaCard}>
